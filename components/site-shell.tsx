    ...
    <>
      ...
      <header className="site-header">
        ...
        <details className="mobile-menu">
          ...
          <nav>{simpleNav.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}<Link href="/documents">Documents</Link><Link href="/guides">Awareness guides</Link><Link href="/company-response">Company response</Link><Link className="button" href="/submit-experience">Share your experience</Link></nav>
        </details>
      </header>
    </>
  );
...
export function Footer() {
  return (
    <footer>
      <div className="footer-grid minimal-footer">
        ...
        <div><h3>Read</h3><Link href="/our-experience">Full customer experience</Link><Link href="/timeline">Timeline</Link><Link href="/reviews">Customer reviews</Link><Link href="/documents">Documents</Link></div>
        <div><h3>Participate</h3><Link href="/submit-experience">Share your experience</Link><Link href="/company-response">Company response</Link><Link href="/contact">Corrections & contact</Link></div>
        <div><h3>Policies</h3>{policies.slice(0, 4).map(([l,s]) => <Link key={s} href={`/policies/${s}`}>{l}</Link>)}</div>
      </div>
