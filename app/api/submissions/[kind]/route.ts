...
export async function POST(request: Request, { params }: { params: Promise<{kind:string}> }) {
  ...
  if (supabase) {
    ...
    const payload = kind === "experience"
      ? { reviewer_name:data.name, public_name:data.publicName || "Anonymous", email:data.email, phone:data.phone, service_type:data.service || "Not specified", service_date:data.serviceDate || null, amount_paid:Number(data.amount)||null, rating:Number(data.rating)||null, title_original:data.title || "Customer experience submission", body_original:data.message, desired_resolution:data.resolution, company_contacted:data.contacted==="Yes", company_responded:data.responded==="Yes", moderation_status:"pending", internal_notes:`Reference ${reference}; abuse hash ${ipHash}` }
      : kind === "response"
